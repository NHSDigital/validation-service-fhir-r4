...
public class HybridTerminologyValidationSupport extends InMemoryTerminologyServerValidationSupport {
    ...
    private List<String> myRemoteCodeSystems = new ArrayList(Arrays.asList("http://snomed.info/sct","https://dmd.nhs.uk"));
    ...
    public CodeValidationResult validateCode(ValidationSupportContext theValidationSupportContext, ConceptValidationOptions theOptions, String theCodeSystem, String theCode, String theDisplay, String theValueSetUrl) {
        log.info("Validate code {} {}",theCodeSystem, theValueSetUrl);
        if (theCodeSystem != null && myRemoteCodeSystems.contains(theCodeSystem) || hasValueSetRemoteCodeSystem(theValidationSupportContext, theValueSetUrl)) return this.validateCodeInValueSetRemote(theValidationSupportContext,
    ...
    public CodeValidationResult validateCodeInValueSet(ValidationSupportContext theValidationSupportContext, ConceptValidationOptions theOptions, String theCodeSystemUrlAndVersion, String theCode, String theDisplay, @NotNull IBaseResource theValueSet) {
        ...
        if (theValueSet != null && theValueSet instanceof ValueSet) {
            log.info("validateCodeInValueSet {} {}",theCodeSystemUrlAndVersion, ((ValueSet) theValueSet).getUrl());
        } else {
            log.info("validateCodeInValueSet {} ",theCodeSystemUrlAndVersion);
        }
        ...
        if (this.hasValueSetRemoteCodeSystem(theValueSet)) return validateCodeInValueSetRemote(theValidationSupportContext,theOptions,theCodeSystemUrlAndVersion, theCode, theDisplay, theValueSet);
    ...
    public boolean hasValueSetRemoteCodeSystem(ValidationSupportContext theValidationSupportContext,String theValueSetUrl) {
        if (theValueSetUrl == null) return false;
        return hasValueSetRemoteCodeSystem(theValidationSupportContext.getRootValidationSupport().fetchValueSet(theValueSetUrl));
    }

    public boolean hasValueSetRemoteCodeSystem(IBaseResource theValueSet) {
        if (theValueSet instanceof ValueSet) {
            ValueSet valueSet = (ValueSet) theValueSet;
            log.info("Checking {}",valueSet.getUrl());
            if (valueSet.hasCompose() && valueSet.getCompose().hasInclude()) {
                for (ValueSet.ConceptSetComponent composeComponent : valueSet.getCompose().getInclude()) {
                    if (composeComponent.hasSystem() && myRemoteCodeSystems.contains(composeComponent.getSystem())) return true;
                }
            }
        }
        return false;
