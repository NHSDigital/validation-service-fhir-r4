...
public class PrePopulatedValidationSupport extends BaseStaticResourceValidationSupport implements IValidationSupport {
     ...
     *                                the resource itself.
     */
    public PrePopulatedValidationSupport(FhirContext theFhirContext, Map<String, IBaseResource> theStructureDefinitions, Map<String, IBaseResource> theValueSets, Map<String, IBaseResource> theCodeSystems,  Map<String, IBaseResource> theOtherConformanceResources) {
    ...
    public void addOtherConformanceResource(IBaseResource theResource) {
        Set<String> urls = processResourceAndReturnUrls(theResource, theResource.getClass().getSimpleName());
        addToMap(theResource, myOtherConformanceResources, urls);
    }
    ...
    public List<IBaseResource> fetchAllConformanceResources() {
        ...
        retVal.addAll(myOtherConformanceResources.values());
        return retVal;
    }
    ...
    public <T extends IBaseResource> T fetchResource(@Nullable Class<T> theClass, String theUri) {
        IBaseResource resource = myOtherConformanceResources.get(theUri);
        if (resource == null) resource = myCodeSystems.get(theUri);
        if (resource == null) resource = myValueSets.get(theUri);
        if (resource == null) resource = myStructureDefinitions.get(theUri);
        return (T) resource;
    }
